using System;
using static BlazeJump.Tools.Helpers.GeneralHelpers;
using BlazeJump.Tools.Enums;
using NUnit.Framework;

namespace BlazeJump.Tools.Tests.HelperTests
{
	[TestFixture]
	public class GeneralHelpersTests
	{
		...
		[TestCase("2021-06-01 00:00:00", 1622505600)]
		public void DateTimeToUnixTimeStamp_ValidInput_ReturnsCorrectUnixTimeStamp(string dateTimeStr, long expectedUnixTime)
		{
			// Arrange
			DateTime dateTime = DateTime.SpecifyKind(DateTime.Parse(dateTimeStr), DateTimeKind.Utc);
		...
		public void HexToBech32_ValidInput_ReturnsEncodedNpub(string hexString, string expectedNpub)
		{
			// Act
			string result = HexToBech32(hexString, Bech32PrefixEnum.npub);
		...
		public void Bech32ToHex_ValidNpub_ReturnsDecodedHex(string npubString, string expectedHex)
		{
			// Act
			string result = Bech32ToHex(npubString, Bech32PrefixEnum.npub);
		...
		public void HexToBech32_ValidInput_ReturnsEncodedNsec(string hexString, string expectedNsec)
		{
			// Act
			string result = HexToBech32(hexString, Bech32PrefixEnum.nsec);
		...
		public void Bech32ToHex_ValidNsec_ReturnsDecodedHex(string nsecString, string expectedHex)
		{
			// Act
			string result = Bech32ToHex(nsecString, Bech32PrefixEnum.nsec);
		...
		[Test]
		public void Bech32ToHex_InvalidNpub_ThrowsException()
		{
			// Act & Assert
			var ex = Assert.Throws<Exception>(() => Bech32ToHex("npub1invalidstring", Bech32PrefixEnum.npub));
			Assert.That(ex.Message, Is.EqualTo("Invalid npub string"));
		}

		[Test]
		public void Bech32ToHex_InvalidNsec_ThrowsException()
		{
			// Act & Assert
			var ex = Assert.Throws<Exception>(() => Bech32ToHex("nsec1invalidstring", Bech32PrefixEnum.nsec));
			Assert.That(ex.Message, Is.EqualTo("Invalid npub string"));
		}
	}
}
